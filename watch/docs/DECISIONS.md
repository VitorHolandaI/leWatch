...
**Alarme diário recorrente** (`ui_alarm.cpp`, app "Alarm"): 1 alarme h:m + switch + som,
salvo no NVS `alarm` (`rec_on/rec_h/rec_m/night_h`). O PCF8563 com dia/semana=0xFF **repete
diário** sozinho — só não pode desabilitar entre as batidas. Boot: `hw_rtc_clear_alarm()`
(desarma → o one-shot do Clock continua one-shot) + `hw_alarm_arm_recurring()` (re-arma o
diário **só** se o recorrente estiver ligado). O ack (`hw_rtc_alarm_ack`, limpa flag sem
desabilitar) é usado no **foreground** (toca acordado sem desarmar o diário).
Dois caminhos de disparo: (1) **acordado** → `ui_poll_timer_callback` chama
`hw_alarm_recurring_fired()` (isAlarmActive) → toca; (2) **ocioso de noite** → o RTC não
acorda light-sleep (WakeupSource só POWER/TOUCH/SENSOR), então na **janela noturna**
[`night_h`:00 → alarme) o idle path chama `hw_enter_alarm_sleep()` (deep sleep até o RTC_INT
→ reboot → toca). De dia = light-sleep normal (glanceável no power-key). Tradeoff: de
madrugada, com deep-sleep armado, ver a hora exige apertar o power (reboot ~2s).
Registrador de alarme do RTC é único → o one-shot do Clock e o recorrente se sobrescrevem
(recorrente é o caminho principal).
