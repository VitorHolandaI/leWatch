...
| `ui_alarm.cpp` | NOVO | App **Alarme diário recorrente**: rollers h:m + switch Ativado + roller "dormir após (h)" + picker de som + Salvar. Config em NVS (`rec_on/rec_h/rec_m/night_h`). |
| `hal_interface.*` | MOD | **(alarme recorrente)** `hw_alarm_cfg_set/get`, `hw_alarm_arm_recurring` (re-arma no boot), `hw_rtc_alarm_ack` (limpa flag sem desabilitar → segue diário), `hw_alarm_recurring_fired`, `hw_alarm_should_deep_sleep` (janela noturna com virada de meia-noite). PCF8563 dia/semana=0xFF repete sozinho. |
| `factory.ino` | MOD | Boot: `hw_rtc_clear_alarm()` (desarma → mantém o one-shot do Clock realmente one-shot) + `hw_alarm_arm_recurring()` (re-arma o diário **só** se o recorrente estiver ligado). |
| `ui_main.cpp` | MOD | `ui_poll_timer_callback`: **foreground ring** (`hw_alarm_recurring_fired` → `ui_alarm_ring`) + **auto deep-sleep noturno** (`hw_alarm_should_deep_sleep` → `hw_enter_alarm_sleep` antes do light-sleep). Célula "Alarm" no launcher. **Colon `:` do relógio fixo** (era escondido/mostrado a cada 1s em `clock_update_datetime` = piscava; agora sempre visível). |
