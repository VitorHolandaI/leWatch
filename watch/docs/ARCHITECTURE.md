...
| Arquivo | App |
|---|---|
| `ui_clock.cpp` | Relógio: cronômetro, countdown, **alarme one-shot** (rollers + MODO ALARME deep-sleep, dropdown de som, ring overlay `ui_alarm_ring`). |
| `ui_alarm.cpp` | **Alarme diário recorrente**: h:m + switch + hora-noite + som, salvo em NVS. RTC repete diário; de noite o relógio auto deep-sleepa pra acordar e tocar. HAL `hw_alarm_cfg_*`/`hw_alarm_should_deep_sleep`/`hw_alarm_recurring_fired`. |
