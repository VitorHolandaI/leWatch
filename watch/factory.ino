...
void setup()
{
    ...
    // Woke from MODO ALARME deep sleep -> show the ringing screen.
    if (hw_woke_from_alarm()) {
        // Disarm first (keeps the one-shot Clock alarm truly one-shot); the
        // recurring alarm, if enabled, is re-armed just below.
        hw_rtc_clear_alarm();
        ui_alarm_ring();
    }
    hw_alarm_arm_recurring();     // re-arm the daily alarm for tomorrow (if enabled)
