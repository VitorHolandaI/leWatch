...
{
    // Recurring alarm firing while the watch is awake: ring in the foreground
    // (the deep-sleep-reboot path handles it when asleep).
    if (!hw_alarm_ringing() && hw_alarm_recurring_fired()) {
    ...
    if (lv_obj_has_flag(main_screen, LV_OBJ_FLAG_HIDDEN)) {
        ...
        // Never light-sleep while recording: it would stall the capture task.
        if (hw_get_disp_is_on() && !hw_record_active()) {
        ...
        if (disp_on && disp_time_ms != 0 && !hw_record_active() && !hw_alarm_ringing()) {
            if (lv_tick_get() > disp_time_ms) {
                ...
                // Inside the night window with a recurring alarm set: deep-sleep
                // until the alarm (light sleep can't wake on the RTC). Does not
                // return — wakes via reboot and rings.
                if (hw_alarm_should_deep_sleep()) {
...
void setupGui()
{
    ...
    extern app_t ui_alarm_main;
    create_app_cell(grid_panel,"Alarm", &img_configuration, &ui_alarm_main);
