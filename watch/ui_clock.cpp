/**
 * @file      ui_clock.cpp
 * @brief     Clock tools app: Stopwatch, Countdown and Alarm (with MODO ALARME).
 *
 * MODO ALARME arms the PCF8563 RTC alarm and deep-sleeps the watch (everything
 * off but the RTC). At the set time RTC_INT wakes the chip; the boot path
 * (factory.ino) detects it via hw_woke_from_alarm() and calls ui_alarm_ring().
 */
#include "ui_define.h"
#include <stdio.h>
#include <string.h>
...
// Som do alarme: dropdown dos audios existentes; guarda a escolha no NVS.
...
static void build_alarm_sound_picker(lv_obj_t *parent)
{
    ...
    // Mesma lista do app de audio: todos os MP3 do SD + FFat, com prefixo da
    // origem para diferenciar arquivos de mesmo nome em filesystems distintos.
    int pos = 0;
    ...
    // Pre-seleciona o som salvo; se nenhum, grava o primeiro como default.
    uint8_t src;
    std::string saved;
    if (hw_alarm_sound_get(src, saved)) {
        for (uint32_t i = 0; i < alarm_music.size(); i++) {
            if (alarm_music[i].file_name == saved) { lv_dropdown_set_selected(dd, i); break; }
        }
    } else {
...
static void alarm_mode_cb(lv_event_t *e)
{
    ...
    lv_refr_now(NULL);
    delay(400);
    hw_enter_alarm_sleep();   // deep sleep, does not return
#else
    (void)h; (void)m;
#endif
}
...
void ui_clock_enter(lv_obj_t *parent)
{
    ...
    // Pré-seleciona a hora/min atuais pra setar o alarme a partir de agora.
    {
        struct tm now = {};
    ...
    build_alarm_sound_picker(page);   // som que toca quando o alarme bate
    make_button(page, "MODO ALARME", alarm_mode_cb, NULL);
