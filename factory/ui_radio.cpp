...
static void select_bandwidth_dropdown()
{
...
static void select_power_dropdown()
{
...
static void refresh_radio_range_options(bool high_freq)
{
    if (_high_freq == high_freq) {
        return;
    }

    lv_dropdown_set_options(power_level_dd, radio_get_tx_power_list(high_freq));
    lv_dropdown_set_options(bandwidth_dd, radio_get_bandwidth_list(high_freq));
...
static void update_radio_frequency(uint16_t selected)
{
    radio_params_copy.freq = radio_get_freq_from_index(selected);
    printf("set freq:%.2f\n", radio_params_copy.freq);
    refresh_radio_range_options(radio_params_copy.freq > 960.0);
    select_bandwidth_dropdown();
    select_power_dropdown();
}

static void apply_radio_run_mode()
{
...
static void back_event_handler(lv_event_t *e)
{
    ...
    if (lv_menu_back_btn_is_root(menu, obj)) {
        ...
        // Radio is idle again until the app reopens; sleep it to save power.
        hw_radio_sleep();
...
static void _ui_radio_obj_event(lv_event_t *e)
{
    ...
    switch (*flag) {
    case 'f':   //*frequency
        update_radio_frequency(selected);
        break;
    case 'w':   //*bandwidth
