...
static const uint16_t k_pmu_charge_table[] = {
    100, 125, 150, 175,
    200, 300, 400, 500,
    600, 700, 800, 900,
    1000
...
static uint16_t hw_safe_max_charger_current()
{
    return USER_MAX_CHARGE_CURRENT_MA < dev_conts_var.max_charge_current ?
           USER_MAX_CHARGE_CURRENT_MA : dev_conts_var.max_charge_current;
}
...
static uint8_t hw_pmu_max_charge_level()
{
    uint16_t max_current = hw_safe_max_charger_current();
    uint8_t max_level = 0;
    for (size_t i = 0; i < sizeof(k_pmu_charge_table) / sizeof(k_pmu_charge_table[0]); ++i) {
...
int random(int min, int max)
{
    ...
    int range = max - min + 1;
    // sim-only Arduino random() shim; rand() randomness is adequate for the simulator
...
#elif defined(USING_AUDIO_CODEC)
    if (HW_CODEC_ONLINE & hw_get_device_online()) {
        int ret = instance.codec.write((uint8_t *)outBuf, mp3_frame_bytes(frameInfo));
        if (ret != 0) {
...
static void mp3_output_frame(int16_t *outBuf, const MP3FrameInfo &frameInfo, bool &output_started)
{
    if (!output_started) {
        output_started = true;
...
static bool mp3_recover_decode_error(int err, const mp3_cursor_t &before, mp3_cursor_t &current)
{
...
{
    if (src_len <= 44 || memcmp(src, "RIFF", 4) != 0 || memcmp(src + 8, "WAVE", 4) != 0) {
        return false;
    }
    ...
    uint32_t declaredDataLen = (uint32_t)src[40] | ((uint32_t)src[41] << 8) |
                               ((uint32_t)src[42] << 16) | ((uint32_t)src[43] << 24);
...
static bool hw_open_audio_file(audio_source_type_t source, const String &path, const char *filename, File &f, bool &locked)
{
    locked = false;
...
static uint8_t *hw_read_audio_file(File &f, const char *filename, size_t &read_size)
{
    read_size = 0;
...
#ifdef ARDUINO
    int ret ;
    if (hw_record_active() || hw_player_running() ||
            !audioMutex || xSemaphoreTake(audioMutex, 0) != pdTRUE) {
        return false;
    }
...
static String hw_record_next_path()
{
    ...
    if (!files.empty()) {
        ...
        seq = (uint32_t)strtoul(last.c_str() + strlen(REC_DIR_PREFIX), NULL, 10) + 1;
    }
    char buf[40];
...
static bool hw_record_enforce_cap()
{
    std::vector<std::string> files;
    hw_record_collect(files);
    for (size_t i = 0; i < files.size() && files.size() - i > REC_MAX_FILES; i++) {
        if (!FFat.remove(files[i].c_str())) return false;
    }
    return true;
}
...
static void recordTask(void *args)
{
    (void)args;
#if defined(USING_AUDIO_CODEC)
    const int inCh = instance.getCodecInputChannels() < 1 ? 1 : instance.getCodecInputChannels();
#else
    const int inCh = 1;
#endif
    ...
    }
    {
...
#if defined(USING_PDM_MICROPHONE)
            capturedBytes = instance.mic.readBytes((char *)mono, FRAMES * sizeof(int16_t));
            if (capturedBytes == 0) {
                result = HW_RECORD_RESULT_CODEC_ERROR;
                break;
            }
#elif defined(USING_AUDIO_CODEC)
            if (HW_CODEC_ONLINE & hw_get_device_online()) {
                if (instance.codec.read((uint8_t *)in, FRAMES * inCh * sizeof(int16_t)) != 0) {
                    result = HW_RECORD_RESULT_CODEC_ERROR;
                    break;
                }
        ...
        f.close();
        if (!keepFile) FFat.remove(recPath);
    }
...
cleanup:
    ...
    (void)opened;
#endif
    if (audioLocked) xSemaphoreGive(audioMutex);
    recResult = result;
    recTaskHandler = NULL;
...
{
#ifdef ARDUINO
    if ((xEventGroupGetBits(recordEvent) & RECORD_ACTIVE) ||
            micHasAudioLock || hw_player_running()) return false;
    if (FFat.freeBytes() < REC_MIN_FREE_BYTES) {
    ...
    recStartMs = millis();
    if (xTaskCreate(recordTask, "app/rec", 8 * 1024, NULL, 11, &recTaskHandler) != pdPASS) {
...
void hw_record_stop()
{
    ...
    if (recordEvent && (xEventGroupGetBits(recordEvent) & RECORD_ACTIVE)) {
        ...
        xEventGroupWaitBits(recordEvent, RECORD_DONE, pdTRUE, pdTRUE, portMAX_DELAY);
        while (xEventGroupGetBits(recordEvent) & RECORD_ACTIVE) delay(1);
    }
#endif
}
...
static void hw_set_default_user_settings(uint8_t brightness, uint8_t keyboard_bl, uint8_t timeout, uint16_t charger_current)
{
...
static void hw_load_user_settings()
{
    if (!prefs.begin(NVS_NAME)) {
        ...
        user_setting.charger_current = hw_clamp_charger_current(hw_get_charger_current());
        return;
    }
    if (prefs.getBytes(NVS_NAME, &user_setting, sizeof(user_setting_params_t)) != sizeof(user_setting_params_t)) {
        ...
        hw_set_default_user_settings(50, 80, 30, DEVICE_CHARGE_CURRENT_RECOMMEND);
        if (prefs.putBytes(NVS_NAME, &user_setting, sizeof(user_setting_params_t)) != sizeof(user_setting_params_t)) {
    ...
    if (user_setting.charger_current != saved_current &&
            prefs.putBytes(NVS_NAME, &user_setting, sizeof(user_setting_params_t)) != sizeof(user_setting_params_t)) {
...
{
    user_setting = param;
    user_setting.charger_current = hw_clamp_charger_current(user_setting.charger_current);
#ifdef ARDUINO
    if (prefs.putBytes(NVS_NAME, &user_setting, sizeof(user_setting_params_t)) != sizeof(user_setting_params_t)) {
...
static void power_key_event_cb(DeviceEvent_t event, void *params, void *user_data)
{
    ...
    if (evt != PMU_EVENT_KEY_CLICKED) return;
...
// wrong password is never saved and the WiFiMulti path (which already reads from
// flash) cannot overwrite a good saved password with a stale one.
...
{
    (void)event;
    if (s_pending_ssid.isEmpty()) return;
    if (WiFi.SSID() != s_pending_ssid) return;
...
#ifdef ARDUINO
static bool prefs_put_string_checked(Preferences &p, const char *key, const char *value)
{
...
static int wifi_saved_find(Preferences &p, uint8_t n, const char *ssid)
{
    for (uint8_t i = 0; i < n; i++) {
        char ks[8]; snprintf(ks, sizeof(ks), "s%u", i);
...
#ifdef ARDUINO
    Preferences p;
    if (!p.begin(WIFI_NS, true)) return 0;
    uint8_t n = p.getUChar("n", 0);
    p.end();
    return n;
#else
...
#ifdef ARDUINO
    Preferences p;
    if (!p.begin(WIFI_NS, true)) return false;
    uint8_t n = p.getUChar("n", 0);
    bool ok = idx < n;
    if (ok) {
        char ks[8]; snprintf(ks, sizeof(ks), "s%u", idx);
...
bool hw_wifi_saved_remove(const char *ssid)
{
    ...
    if (!p.remove(ksl)) log_w("Failed to remove saved WiFi SSID tail");
    if (!p.remove(kpl)) log_w("Failed to remove saved WiFi password tail");
    bool ok = p.putUChar("n", n - 1) == 1;
    p.end();
    return ok;
#else
    for (auto it = g_sim_nets.begin(); it != g_sim_nets.end(); ++it) {
        if (it->first == ssid) { g_sim_nets.erase(it); return true; }
    }
    return false;
#endif
}
...
static void wifi_connect_task(void *arg)
{
    ...
    if (p.begin(WIFI_NS, true)) {
        ...
        for (uint8_t i = 0; i < n; i++) {
            ...
            if (s.length()) wm.addAP(s.c_str(), pw.c_str());
        }
        p.end();
    }
...
{
#ifdef ARDUINO
    if (s_wifi_connecting) return true;   // ja em andamento
    if (hw_wifi_saved_count() == 0) return false;
...
#if defined(USING_PPM_MANAGE)
    if (enable) {
        instance.ppm.setChargerConstantCurr(hw_clamp_charger_current(user_setting.charger_current));
...
#elif defined(USING_PMU_MANAGE)
    if (enable) {
        instance.enableCharge(hw_clamp_charger_current(user_setting.charger_current));
    } else {
...
{
    // clamped value is consumed by the PMU calls below on hardware; sim has no PMU
...
uint8_t hw_get_charger_current_level()
{
    ...
    if (dev_conts_var.charge_steps == 0) return 0;
    return (current + dev_conts_var.charge_steps - 1) / dev_conts_var.charge_steps;
#elif defined(USING_PMU_MANAGE)
    uint16_t cur = hw_clamp_charger_current(instance.getChargeCurrent());
    for (int i = 0; i < sizeof(k_pmu_charge_table) / sizeof(k_pmu_charge_table[0]); ++i) {
...
{
#if defined(USING_PPM_MANAGE)
    uint16_t current = hw_clamp_charger_current(level * dev_conts_var.charge_steps);
#else
...
#else
    params.type = MONITOR_PPM;
    // sim-only fake battery reading; rand() randomness is adequate for the simulator
    // NOLINTNEXTLINE(cert-msc30-c,cert-msc50-cpp)
...
void hw_pedometer_start()
{
    ...
    if (hw_get_device_online() & HW_BMA423_ONLINE) {
        ...
        instance.sensor.configAccelerometer(SensorBMA423::RANGE_2G,
                                            SensorBMA423::ODR_50HZ,
                                            SensorBMA423::BW_NORMAL_AVG4,
                                            SensorBMA423::PERF_CIC_AVG_MODE);
        instance.sensor.enableAccelerometer();
...
// download them. Extracts the WAV memos off the FFat flash (no SD, no USB path).
#ifdef ARDUINO
#include <WebServer.h>
...
{
    if (s_share_server->authenticate(SHARE_HTTP_USER, s_share_http_pass.c_str())) {
        return true;
    }
...
bool hw_share_start()
{
    ...
    if (xTaskCreate(share_task, "app/share", 6144, NULL, 5, &s_share_task) != pdPASS) {
...
{
    return hw_safe_max_charger_current();
}
...
#if defined(USING_PPM_MANAGE)
    if (dev_conts_var.charge_steps == 0) return 0;
    uint8_t max_level = hw_safe_max_charger_current() / dev_conts_var.charge_steps;
    return max_level < dev_conts_var.charge_level_nums ? max_level : dev_conts_var.charge_level_nums;
#elif defined(USING_PMU_MANAGE)
    return hw_pmu_max_charge_level();
#else
    return hw_pmu_max_charge_level();
...
// rebrand from (Consul=Kelvinator, Elgin=Gree, Springer=Midea, Philco=TCL112...).
// Uses IRremoteESP8266's unified IRac so one path covers every vendor. The UI
// steps through these one at a time showing the name, so the user can see which
// protocol actually turned the unit on.
// Order kept in sync with the decode_type_t list in hw_ac_blast_send_on(). Many
// brands span several protocol families (Daikin, LG, TCL), so they get an entry
// each. Names hint the common Brazilian retail brands per family.
static const char *s_ac_blast_names[] = {
    "Midea/Springer/Komeco", "Coolix", "Gree/Elgin", "Kelvinator/Consul",
    "TCL112/Philco", "TCL96/Electrolux", "Neoclima/Komeco", "Whirlpool/Consul",
    "Samsung WindFree", "LG", "LG Dual Inverter", "Daikin", "Daikin2",
    "Daikin128", "Daikin152", "Daikin160", "Daikin176", "Fujitsu",
    "Electra", "Toshiba", "Carrier", "Hitachi", "Panasonic", "Haier",
    "Mitsubishi",
};
...
void hw_ac_blast_send_on(uint8_t i)
{
    ...
    static const decode_type_t protos[] = {
        MIDEA, COOLIX, GREE, KELVINATOR,
        TCL112AC, TCL96AC, NEOCLIMA, WHIRLPOOL_AC,
        SAMSUNG_AC, LG, LG2, DAIKIN, DAIKIN2,
        DAIKIN128, DAIKIN152, DAIKIN160, DAIKIN176, FUJITSU_AC,
        ELECTRA_AC, TOSHIBA_AC, CARRIER_AC, HITACHI_AC, PANASONIC_AC, HAIER_AC,
        MITSUBISHI_AC,
    };
...
// mode/fan: AcMode/AcFan de ir_control.h (COOL=0,HEAT=1,FAN=2 / LOW=0,MID=1,HIGH=2,AUTO=3).
void hw_ac_electra_send(bool power, uint8_t temp, uint8_t mode, uint8_t fan)
{
    ...
    uint8_t m;
    switch (mode) {
        case 1:  m = kElectraAcHeat; break;   // ACM_HEAT
        case 2:  m = kElectraAcFan;  break;   // ACM_FAN
        default: m = kElectraAcCool;          // ACM_COOL
    }
    ac.setMode(m);

    uint8_t f;
    switch (fan) {
        case 0:  f = kElectraAcFanLow;  break;  // ACF_LOW
        case 1:  f = kElectraAcFanMed;  break;  // ACF_MID
        case 2:  f = kElectraAcFanHigh; break;  // ACF_HIGH
        default: f = kElectraAcFanAuto;         // ACF_AUTO
    }
    ac.setFan(f);

    ac.send();
#else
    (void)power; (void)temp; (void)mode; (void)fan;
#endif
}
...
void hw_ac_midea_send(bool power, uint8_t temp, uint8_t mode, uint8_t fan, bool swing_toggle)
{
    ...
    stdAc::opmode_t m;
    switch (mode) {
        case 1:  m = stdAc::opmode_t::kHeat; break;  // ACM_HEAT
        case 2:  m = stdAc::opmode_t::kFan;  break;  // ACM_FAN
        default: m = stdAc::opmode_t::kCool;          // ACM_COOL
    }

    stdAc::fanspeed_t f;
    switch (fan) {
        case 0:  f = stdAc::fanspeed_t::kLow;    break;  // ACF_LOW
        case 1:  f = stdAc::fanspeed_t::kMedium; break;  // ACF_MID
        case 2:  f = stdAc::fanspeed_t::kHigh;   break;  // ACF_HIGH
        default: f = stdAc::fanspeed_t::kAuto;           // ACF_AUTO
    }
    ...
    irac.next.swingv   = swing_toggle ? stdAc::swingv_t::kAuto : stdAc::swingv_t::kOff;
    irac.next.swingh   = stdAc::swingh_t::kOff;
    irac.next.light    = false;
    irac.next.beep     = false;
    irac.next.econo    = false;
    ...
    irac.sendAc();
#else
    (void)power; (void)temp; (void)mode; (void)fan; (void)swing_toggle;
#endif
}
...
void hw_ac_lg_send(bool power, uint8_t temp, uint8_t mode, uint8_t fan, bool swing, bool swing_toggle)
{
    ...
    if (!began) {
        began = true;
        ac.begin();
        ac.setModel(AKB74955603);  // LG2 split AC variant common in IRremoteESP8266 notes.
    }
    ...
    uint8_t m;
    switch (mode) {
        case 1:  m = kLgAcHeat; break;   // ACM_HEAT
        case 2:  m = kLgAcFan;  break;   // ACM_FAN
        default: m = kLgAcCool;          // ACM_COOL
    }
    ac.setMode(m);

    uint8_t f;
    switch (fan) {
        case 0:  f = kLgAcFanLow;    break;  // ACF_LOW
        case 1:  f = kLgAcFanMedium; break;  // ACF_MID
        case 2:  f = kLgAcFanHigh;   break;  // ACF_HIGH
        default: f = kLgAcFanAuto;          // ACF_AUTO
    }
    ac.setFan(f);
    ...
    ac.send();
#else
    (void)power; (void)temp; (void)mode; (void)fan; (void)swing; (void)swing_toggle;
#endif
}
...
void hw_ac_samsung_send(bool power, uint8_t temp, uint8_t mode, uint8_t fan, bool swing)
{
    ...
    uint8_t m;
    switch (mode) {
        case 1:  m = kSamsungAcHeat; break;   // ACM_HEAT
        case 2:  m = kSamsungAcFan;  break;   // ACM_FAN
        default: m = kSamsungAcCool;          // ACM_COOL
    }
    ac.setMode(m);

    uint8_t f;
    switch (fan) {
        case 0:  f = kSamsungAcFanLow;  break;  // ACF_LOW
        case 1:  f = kSamsungAcFanMed;  break;  // ACF_MID
        case 2:  f = kSamsungAcFanHigh; break;  // ACF_HIGH
        default: f = kSamsungAcFanAuto;         // ACF_AUTO
    }
    ac.setFan(f);
    ac.setSwing(swing);
    ac.send();
#else
    (void)power; (void)temp; (void)mode; (void)fan; (void)swing;
#endif
}
...
void hw_ac_toshiba_send(bool power, uint8_t temp, uint8_t mode, uint8_t fan, bool swing, bool swing_toggle)
{
    ...
    uint8_t m;
    switch (mode) {
        case 1:  m = kToshibaAcHeat; break;   // ACM_HEAT
        case 2:  m = kToshibaAcFan;  break;   // ACM_FAN
        default: m = kToshibaAcCool;          // ACM_COOL
    }
    ac.setMode(m);
    ac.setPower(power);
    ac.setTemp(temp);
    ...
    uint8_t f;
    switch (fan) {
        case 0:  f = kToshibaAcFanMin;  break;  // ACF_LOW
        case 1:  f = kToshibaAcFanMed;  break;  // ACF_MID
        case 2:  f = kToshibaAcFanMax;  break;  // ACF_HIGH
        default: f = kToshibaAcFanAuto;         // ACF_AUTO
    }
    ac.setFan(f);
                                ...
                                WiFiClient &client)
{
    ...
    while (http.connected() && n < out_size - 1 && (int32_t)(deadline - millis()) > 0) {
...
#else
    (void)url; (void)out; (void)out_size;
    return -1;
#endif
}
...
{
#if defined(ARDUINO)
    if (out == NULL || out_size == 0 || body == NULL) return -1;
    out[0] = 0;
    if (WiFi.status() != WL_CONNECTED) return -1;
    ...
    WiFiClientSecure secure_client;
    WiFiClient plain_client;
    if (https) secure_client.setInsecure();   // skip certificate validation
    WiFiClient &client = https ? (WiFiClient &)secure_client : plain_client;
    ...
    return (int)n;
#else
    (void)url; (void)body; (void)out; (void)out_size;
    return -1;
#endif
}

bool hw_fs_write_file(const char *path, const char *data, uint32_t len)
{
#if defined(ARDUINO)
