...
// True if this boot was caused by the normal RTC alarm firing.
bool hw_woke_from_alarm();
// Normal one-shot alarm: sleep until the RTC alarm, then reboot and ring.
...
/**
 * @brief Blocking HTTP POST of a JSON body into a buffer (for MCP tools/call).
 *
 * Sends @p body as the request payload with headers
 * `Content-Type: application/json` and
 * `Accept: application/json, text/event-stream`, then copies up to out_size-1
 * bytes of the raw response body (which may be SSE-framed) into @p out,
 * NUL-terminated and truncated if larger. HTTPS certificates are not validated.
 * Blocks for the duration of the request, so call it from a task.
 *
 * @param url      Full URL (http or https).
 * @param body     Request body (JSON-RPC tools/call), NUL-terminated.
 * @param out      Destination buffer.
 * @param out_size Size of out in bytes.
 * @return Number of bytes copied, or -1 on error.
 */
...
/**
 * @brief Read a whole file from the internal filesystem into @p out.
