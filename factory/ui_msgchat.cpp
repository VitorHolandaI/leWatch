...
static bool handle_msg_input_event(lv_indev_t *indev, lv_event_code_t code)
{
    if (indev->type == LV_INDEV_TYPE_KEYPAD) {
        return false;
    }

    if (indev->type == LV_INDEV_TYPE_POINTER && code == LV_EVENT_VALUE_CHANGED) {
        hw_feedback();
    }
    return true;
}

static void handle_msg_key(lv_event_t *e, lv_event_code_t code)
{
...
static bool handle_msg_click(lv_obj_t *ta, lv_indev_t *indev, bool edited)
{
    if (indev->type == LV_INDEV_TYPE_POINTER) {
        lv_group_set_editing((lv_group_t *)lv_obj_get_group(ta), true);
#ifdef USING_TOUCHPAD
        show_msg_keyboard(ta);
#endif
        return true;
    }

    if (indev->type == LV_INDEV_TYPE_ENCODER && edited) {
...
static void _msg_ta_cb(lv_event_t *e)
{
    ...
    if (!handle_msg_input_event(indev, code)) {
        return;
    }

    handle_msg_key(e, code);
    ...
    if (code == LV_EVENT_CLICKED && handle_msg_click(ta, indev, edited)) {
        return;
    }

    if (code == LV_EVENT_FOCUSED && edited) {
        enable_keyboard();
    }
}
