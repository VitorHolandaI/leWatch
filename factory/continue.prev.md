...
- Task Weather/News subiu para 24KB e falha de `xTaskCreate` mostra erro em vez
  de ficar em loading.
- Alarme recorrente cacheia config NVS para não abrir Preferences no poll.
...
8. **App Alarme diário recorrente** (`ui_alarm.cpp`, célula "Alarm"): h:m + switch
   + "dormir após (h)" + som, salvo NVS (`rec_on/rec_h/rec_m/night_h`).
   - PCF8563 dia/semana=0xFF **repete diário** no HW.
   - HAL: `hw_alarm_cfg_set/get`, `hw_alarm_arm_recurring` (boot), `hw_rtc_alarm_ack`
     (limpa flag SEM desabilitar), `hw_alarm_recurring_fired`,
     `hw_alarm_should_deep_sleep` (janela noturna com virada de meia-noite).
   - Disparo: **acordado** → `ui_poll_timer_callback` foreground-ring; **ocioso de
     noite** (janela [`night_h`:00 → alarme)) → `hw_enter_alarm_sleep` (deep sleep,
     acorda por reboot no RTC_INT; light-sleep NÃO acorda no RTC).
   - Boot (`factory.ino`): `hw_rtc_clear_alarm()` (mantém o one-shot do Clock
     realmente one-shot) + `hw_alarm_arm_recurring()` (re-arma só se recorrente on).
   - Clock mantém o alarme **one-shot** (MODO ALARME). Os dois dividem o único
     registrador do RTC → usar juntos se sobrescreve (recorrente = principal).
...
10. **Weather/News mais estável**: Open-Meteo usa HTTP direto; News usa Tom's
   ...
   Config do alarme recorrente agora fica cacheada, evitando abrir NVS no poll.
...
- Alarme (one-shot e recorrente) toca; recorrente persiste no reboot (NVS) e
  dispara acordado (foreground) e de madrugada (deep-sleep→reboot).
...
- Ícone dedicado p/ Share/Recorder/Alarm (hoje reusam img_wifi/microphone/config).
...
Plano do alarme recorrente: `~/.claude/plans/compiled-tickling-stearns.md`.
