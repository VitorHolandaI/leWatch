...
void hw_radio_begin()
{
    ...
    // instance.begin() leaves the SX1262 in standby (~1.5 mA). Nothing uses the
    // radio until a LoRa app opens, so drop it to sleep (~1 uA) right away.
    instance.lockSPI();
    radio.sleep();
    instance.unlockSPI();
}

static int16_t apply_radio_mode(uint8_t mode);
#endif

static const char *radio_mode_name(uint8_t mode)
{
    switch (mode) {
    case RADIO_DISABLE: return "RADIO_DISABLE";
    case RADIO_TX:      return "RADIO_TX";
    case RADIO_RX:      return "RADIO_RX";
    case RADIO_CW:      return "RADIO_CW";
    default:            return "";
    }
}
...
static int16_t apply_radio_mode(uint8_t mode)
{
    printf("Mode: %s\n", radio_mode_name(mode));
    switch (mode) {
    case RADIO_DISABLE: return radio.standby();
    case RADIO_TX:      return radio.startTransmit("");
    case RADIO_RX:      return radio.startReceive();
    case RADIO_CW:      return 0;
    default:            return 0;
    }
}
#endif
...
int16_t hw_set_radio_params(radio_params_t &params)
{
    ...
    printf("SF:%u \n", params.sf);
    printf("SyncWord:%u \n", params.syncWord);
    printf("Mode: %s\n", radio_mode_name(params.mode));

#ifdef ARDUINO
    int16_t state = 0;
    instance.lockSPI();
    // Wake from sleep (no-op if already in standby) so the config below applies.
    radio.standby();
    ...
    state = apply_radio_mode(params.mode);
    instance.unlockSPI();
    return state;
#else
    return 0;
