...
static esp_err_t begin_in_lwip_ctx(void *ctx) {
	...
	// PATCH (T-Watch): do NOT allocate a netif client-data slot. The Arduino
	// ESP32 core builds lwIP with LWIP_NUM_NETIF_CLIENT_DATA = 0, so this asserts
	// and reboots. wireguardif.c stores the device in a single static instead
	// (this firmware runs exactly one WireGuard netif). wg_netif_client_id stays
	// unused (0xFF). See the matching patch in wireguardif.c.
